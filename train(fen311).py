...
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=0)  # 验证集不需要shuffle
test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)  # 测试集不需要shuffle

# 创建保存数据集的文件夹
dataset_save_dir = r"D:\深度学习（豌豆）\mobilenetV-3\saved_datasets"
...
def save_dataset(dataset, save_dir, dataset_type):
    ...
    # 获取类别到文件夹的映射
    class_to_folder = {v: k for k, v in flower_list.items()}
    ...
    for idx in range(len(dataset)):
        ...
        # 创建类别文件夹
        class_folder = os.path.join(save_dir, dataset_type, class_to_folder[label])
        os.makedirs(class_folder, exist_ok=True)

        # 复制图像到对应文件夹
        shutil.copy2(img_path, os.path.join(class_folder, img_name))


# 保存训练集、验证集和测试集
print("Saving datasets...")
save_dataset(train_dataset, dataset_save_dir, "train")
save_dataset(val_dataset, dataset_save_dir, "val")
save_dataset(test_dataset, dataset_save_dir, "test")
